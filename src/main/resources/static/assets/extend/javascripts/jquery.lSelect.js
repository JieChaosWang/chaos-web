        ...
        lSelect: function (options) {
            ...
            return this.each(function () {
                ...
                function addSelect($position, parentId, currentId) {
                    ...
                    //只显示两级
                    if ($.find("select[name=" + selectName + "]").length == 2) {
                        $input.val(parentId);
                        if (typeof(queryScenicSopt) == "function" && $.isFunction(queryScenicSopt)) {
                            queryScenicSopt();
                        }
                        return false;
                    } else {
                    ...
                    var select = '<select name="' + selectName + '" class="form-control">';
